### MODIFIED __init__.py FILE FOR LUMA.OLED TO MAKE CH1116 DISPLAY WORK SH1106 CLASS (SEE LINE 206) ###
...
class ch1115(device):
    ...
    def __init__(self, serial_interface=None, width=128, height=64,
                 rotate=0, **kwargs):
        # Re-use the SSD1306 command set â€“ CH1115 is largely compatible
...
class sh1106(device):
    ...
    def display(self, image):
        ...
        for y in range(0, int(self._pages * pixels_per_page), pixels_per_page):
            #########################################################################################
            self.command(set_page_address, 0x00, 0x10) ### MANUAL CHANGE FROM 0x02 TO 0x00 FOR CH1116
            #########################################################################################
            set_page_address += 1
...
class sh1107(device):
    ...
    :param rotate: An integer value of 0 (default), 1, 2 or 3 only, where 0 is
        no rotation, 1 is rotate 90Â° clockwise, 2 is 180Â° rotation and 3
        represents 270Â° rotation.
    :type rotate: int
...
class ssd1315(ssd1306):
    """
    Serial interface to a monochrome SSD1315 OLED display.
...
class ws0010(parallel_device, character, __framebuffer_mixin):
    ...
    :param exec_time: Time in seconds to wait for a command to complete.
        Default is 50 Î¼s (1e-6 * 50) which is enough for all ws0010 commands.
